...
pub fn play_round(bet: i32, deck: &mut Deck) -> i32 {
    ...
    while !player_hand.is_bust() {
        ...
        println!("Your hand: {:?}", player_hand.cards().iter().map(|card| card.display()).collect::<Vec<String>>());
        println!();
        println!("Dealer's hand: {:?}", dealer_hand.first_card().map(|card| card.display()).unwrap_or("Unknown".to_string()));
        println!("Do you want to hit or stand? (h/s)");
        let mut choice = String::new();
        std::io::stdin().read_line(&mut choice).unwrap();
        if choice.trim().to_lowercase() == "h" {
            player_hand.hit(deck.deal().unwrap());
            println!("You drew: {:?}", player_hand.cards().last().unwrap().display());
        } else {
            break;
        }
    }
    ...
    if player_hand.is_bust() {
        ...
        while dealer_hand.value() < 17 {
            dealer_hand.hit(deck.deal().unwrap());
            println!("Dealer drew: {:?}", dealer_hand.cards().last().unwrap().display());
            println!("Dealer's hand: {:?}", dealer_hand.value());
        }
